...
const BrandUsersTable = () => {
  ...
  // const { role = '' } = useSelector((state: RootState) => state.auth); // Default to empty string if role is undefined
  ...
  const handleViewReferralsClick = (userId: string) => {
    // If user is admin, they can view referrals for any user
    // If user is a brand, they can only view their own referrals
    const url = role === "admin" ? `/users/${userId}/referrals` : `/brand/${userId}/referrals`
    router.push(url)
  }
